...
def build_query_str(options):
    query_str = r"select raw_log:body from log.viewer_stats_log_json where raw_log:header:time >= '{}' and raw_log:body:agent:version like 'Second Life%'and raw_log:body:preferences is not NULL limit {}".format(options.query_date,options.query_max)
    print query_str
    return query_str

def query_snowflake(query_str):
    recs = []
...
def get_records(options):
    ...
    if options.use_query:
        query_str = build_query_str(options)
        recs += query_snowflake(query_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--show_unused", action="store_true", help="show info about unused preferences")
    ...
    if options.preferences:
        ...
        if options.remove_unused or options.show_unused:
