...
integer change_gstate(string gstate)
{
    ...
    if (gstate=="default")
    {
        say_if_verbose("alter all prims to default");
        llSetLinkPrimitiveParamsFast(LINK_SET, [
            PRIM_COLOR, face, <1,1,1>, 1,
            PRIM_GLOW, face, 0,
            PRIM_BUMP_SHINY, face, PRIM_SHINY_NONE, PRIM_BUMP_NONE,
            PRIM_POINT_LIGHT, 0, <0,1,0>, 1.0, 10.0, 0.5,
            PRIM_NORMAL, face, NULL_KEY, <1,1,0>, <0,0,0>, 0,
            PRIM_SPECULAR, face, NULL_KEY, <1,1,0>, <0,0,0>, 0, <1,1,1>, 200, 0
        ]);
    }
    else if (gstate=="alpha")
    {
        vector color = <1,0,0>;
        float alpha = 0.5;
...
integer clear_gstate(string gstate)
{
    ...
    if (gstate=="default")
    {
        say_if_verbose("clear default state, nothing to do"); 
    }
    else if (gstate=="alpha")
    {
        vector color = <1,1,1>;
