...
LLImageDataLock::LLImageDataLock(const LLImageRaw* image)
: mImage(nullptr)
{
    if (image)
    {
        LLSharedMutex* mutex = &const_cast<LLImageRaw*>(image)->mDataMutex;
        if (mutex->isLocked() && !mutex->isThreadLocked())
        {
            if (!mutex->isShared())
            {
                LL_ERRS() << "Mutex is locked exclusively and can't be locked exclusively" << LL_ENDL;
            }
            else
            {
                LL_ERRS() << "Mutex is locked and can't be locked exclusively" << LL_ENDL;
            }
        }
        update(image);
    }
...
LLImageDataLock::LLImageDataLock(const LLImageFormatted* image)
: mImage(nullptr)
{
    if (image)
    {
...
LLImageDataLock::~LLImageDataLock()
{
    if (mImage)
    {
        update(nullptr);
...
void LLImageDataLock::update(const LLImageBase* image)
{
    if (image == mImage)
        return;
    ...
    if (mImage)
    {
        mImage->mDataMutex.unlock();
    }

    mImage = const_cast<LLImageBase*>(image);

    if (mImage)
    {
        mImage->mDataMutex.lock();
    }
}

//============================================================================

LLImageDataSharedLock::LLImageDataSharedLock(const LLImageRaw* image)
: mImage(nullptr)
{
    if (image)
    {
        LLSharedMutex* mutex = &const_cast<LLImageRaw*>(image)->mDataMutex;
        if (!mutex->isShared() && mutex->isLocked() && !mutex->isThreadLocked())
        {
            LL_ERRS() << "Mutex is locked exclusively and can't be locked" << LL_ENDL;
        }
        update(image);
    }
}

LLImageDataSharedLock::LLImageDataSharedLock(const LLImageFormatted* image)
: mImage(nullptr)
{
    if (image)
    {
...
LLImageDataSharedLock::~LLImageDataSharedLock()
{
    if (mImage)
    {
        update(nullptr);
...
void LLImageDataSharedLock::update(const LLImageBase* image)
{
    if (image == mImage)
        return;
    ...
    if (mImage)
    {
        mImage->mDataMutex.unlockShared();
    }
