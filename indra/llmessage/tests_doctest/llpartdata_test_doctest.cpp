...
namespace tut
{
    ...
    inline void ensure_approximately_equals(const std::string& message, F32 actual, F32 expected, S32 tolerance_power)
    {
        const F32 tolerance = std::pow(10.0f, -static_cast<F32>(tolerance_power));
        CHECK_MESSAGE(std::fabs(actual - expected) <= tolerance, message.c_str());
    }
...
TUT_SUITE("LLPartData")
{
    TUT_CASE("LLPartData::partdata_test_object_t_test_1")
    {
        using namespace tut;
        ...
        ensure_approximately_equals("mAngularVelocity.mV[1] different after unpacking", llpsysdata.mAngularVelocity.mV[1], 0.f, 8);
        ensure_approximately_equals("mAngularVelocity.mV[2] different after unpacking", llpsysdata.mAngularVelocity.mV[2], 0.f, 8);
