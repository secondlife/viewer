// doctest translation of alignment_test.cpp
#include "doctest.h"
...
TUT_SUITE("alignment_test")
{
    ...
    TUT_CASE("alignment_test::alignment_test_object_test_1")
    {
        using namespace tut;
...
#       endif
        ...
        for (int i=0; i<num_tests; i++)
        {
            ...
            TUT_ENSURE("ll_aligned_malloc_16 failed", is_aligned(align_ptr,16));

            align_ptr = ll_aligned_realloc_16(align_ptr,2*sizeof(MyVector4a), sizeof(MyVector4a));
            TUT_ENSURE("ll_aligned_realloc_16 failed", is_aligned(align_ptr,16));

            ll_aligned_free_16(align_ptr);

            align_ptr = ll_aligned_malloc_32(sizeof(MyVector4a));
            TUT_ENSURE("ll_aligned_malloc_32 failed", is_aligned(align_ptr,32));
            ll_aligned_free_32(align_ptr);
        }
    }

    // In-place allocation of objects and arrays.
    TUT_CASE("alignment_test::alignment_test_object_test_2")
    {
        using namespace tut;

        MyVector4a vec1;
        TUT_ENSURE("LLAlignment vec1 unaligned", is_aligned(&vec1,16));

        MyVector4a veca[12];
        TUT_ENSURE("LLAlignment veca unaligned", is_aligned(veca,16));
    }

    // Heap allocation of objects and arrays.
    TUT_CASE("alignment_test::alignment_test_object_test_3")
    {
        using namespace tut;
...
#       endif
        ...
        for(int i=0; i<ARR_SIZE; i++)
        {
            ...
            TUT_ENSURE("LLAlignment vecp unaligned", is_aligned(vecp,16));
            delete vecp;
        }
