...
{
    if ((persist != ePersist::PERSIST_NO) && mComment.empty())
    {
...
bool LLControlVariable::shouldSave(bool nondefault_only) const
{
    ...
    if (mPersist == ePersist::PERSIST_NO)
        return false;

    if (mPersist == ePersist::PERSIST_ALWAYS)
        return true;

    // ePersist::ePersist::PERSIST_NONDFT
    // If caller doesn't need us to filter, just save.
    if (! nondefault_only)
        return true;

    // ePersist::ePersist::PERSIST_NONDFT: caller only wants us to save this variable if its value
...
LLControlVariable* LLControlGroup::declareControl(const std::string& name, eControlType type, const LLSD initial_val, const std::string& comment, LLControlVariable::ePersist persist, bool hidefromsettingseditor)
{
    ...
    if (existing_control)
    {
        if ((persist != LLControlVariable::ePersist::PERSIST_NO) && existing_control->isType(type))
        {
...
U32 LLControlGroup::loadFromFileLegacy(const std::string& filename, bool require_declaration, eControlType declare_as)
{
    ...
    while(child_nodep)
    {
        ...
        if (!declared)
        {
            ...
            {
            case TYPE_COL4:
                declareColor4(name, LLColor4::white, LLStringUtil::null, LLControlVariable::ePersist::PERSIST_NO);
                break;
            case TYPE_STRING:
            default:
                declareString(name, LLStringUtil::null, LLStringUtil::null, LLControlVariable::ePersist::PERSIST_NO);
                break;
            }
        }
...
U32 LLControlGroup::loadFromFile(const std::string& filename, bool set_default_values, bool save_values, bool error_when_no_comment)
{
    ...
    {
        LLControlVariable::ePersist persist = LLControlVariable::ePersist::PERSIST_NONDFT;
        ...
        if(control_map.has("Persist"))
        {
            persist = control_map["Persist"].asInteger()?
                      LLControlVariable::ePersist::PERSIST_NONDFT : LLControlVariable::ePersist::PERSIST_NO;
        }
        ...
        else
        {
            ...
            if (! set_default_values)
            {
                // Using ePersist::PERSIST_ALWAYS insists that saveToFile() (which calls
                ...
                persist = LLControlVariable::ePersist::PERSIST_ALWAYS;
                ...
                && comment.empty())
            {
                ...
                // for the next run.
                persist = LLControlVariable::ePersist::PERSIST_NO;
                comment = "Comment not provided, setting won't persist";
                LL_WARNS() << "Control " << name << " is missing a comment value. Setting will be marked as ePersist::PERSIST_NO" << LL_ENDL;
            }
