...
TUT_SUITE("control_group")
{
    TUT_CASE("control_group::control_group_t_test_1")
    {
        using namespace tut;
        control_group data;
        auto& mCG = data.mCG;
        auto& mTestConfigFile = data.mTestConfigFile;
    ...
    TUT_CASE("control_group::control_group_t_test_2")
    {
        using namespace tut;
        control_group data;
        auto& mCG = data.mCG;
        auto& mTestConfigFile = data.mTestConfigFile;
        auto& mTestConfigDir = data.mTestConfigDir;
        auto& mCleanups = data.mCleanups;
        int results = mCG->loadFromFile(mTestConfigFile.c_str());
    ...
    //priorities
    TUT_CASE("control_group::control_group_t_test_3")
    {
        using namespace tut;
        control_group data;
        auto& mCG = data.mCG;
    ...
    //listeners
    TUT_CASE("control_group::control_group_t_test_4")
    {
        using namespace tut;
        control_group data;
        auto& mCG = data.mCG;
        auto& mTestConfigFile = data.mTestConfigFile;
        auto& mListenerFired = control_group::mListenerFired;
        int results = mCG->loadFromFile(mTestConfigFile.c_str());
        ensure("number of settings", (results == 1));
        mCG->getControl("TestSetting")->getSignal()->connect(boost::bind(&control_group::handleListenerTest));
        mCG->setU32("TestSetting", 13);
