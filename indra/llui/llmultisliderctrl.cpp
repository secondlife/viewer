    ...
    mTextDisabledColor(p.text_disabled_color())
{
    ...
    if (p.show_text)
    {
        ...
        if( p.can_edit_text )
        {
            ...
            mEditor->setFocusReceivedCallback( [this](LLFocusableElement* caller) { LLMultiSliderCtrl::onEditorGainFocus(caller, this); } );
...
void LLMultiSliderCtrl::updateText()
{
    if( mEditor || mTextBox )
    {
        ...
        F32 displayed_value = static_cast<F32>(floor(getCurSliderValue() * pow(10.0, static_cast<F64>(mPrecision)) + 0.5) / pow(10.0, static_cast<F64>(mPrecision)));
