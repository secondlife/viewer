...
namespace tut
{
    ...
    void sd_object::test<1>()
    {
        ...
        LLMemoryStream mstr(std::span<const U8>(reinterpret_cast<const U8*>(str.c_str()), str.size()));
        LLSD response;
...
namespace tut
{
    ...
    void mem_object::test<1>()
    {
        ...
        LLMemoryStream mem(std::span<const U8>(reinterpret_cast<const U8*>(&HELLO_WORLD[0]), strlen(HELLO_WORLD)));      /* Flawfinder: ignore */
        std::string hello;
        std::string world;
        mem >> hello >> world;
