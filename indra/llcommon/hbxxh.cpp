...
{
    return XXH3_64bits(static_cast<const void*>(str), strlen(str));
}

//static
U64 HBXXH64::digest(const std::string& str)
{
    return XXH3_64bits(static_cast<const void*>(str.c_str()), str.size());
}
...
{
    mDigest = 0;
    mState = static_cast<void*>(XXH3_createState());
    if (!mState || XXH3_64bits_reset(static_cast<XXH3_state_t*>(mState)) != XXH_OK)
    {
...
HBXXH64::~HBXXH64()
{
    if (mState)
    {
        XXH3_freeState(static_cast<XXH3_state_t*>(mState));
    }
}
...
void HBXXH64::update(const void* buffer, size_t len)
{
    if (mState)
    {
        XXH3_64bits_update(static_cast<XXH3_state_t*>(mState), buffer, len);
    }
    else
    {
...
void HBXXH64::update(const std::string& str)
{
    if (mState)
    {
        XXH3_64bits_update(static_cast<XXH3_state_t*>(mState), static_cast<const void*>(str.c_str()),
                           str.length());
    }
    else
    {
...
void HBXXH64::update(std::istream& stream)
{
    ...
    while (stream.good())
    {
        ...
        len = stream.gcount();
        XXH3_64bits_update(static_cast<XXH3_state_t*>(mState), static_cast<const void*>(buffer), len);
    }
}
...
void HBXXH64::update(FILE* file)
{
    ...
    char buffer[BLOCK_LEN];
    size_t len;
    while ((len = fread(static_cast<void*>(buffer), 1, BLOCK_LEN, file)))
    {
        XXH3_64bits_update(static_cast<XXH3_state_t*>(mState), static_cast<const void*>(buffer), len);
    }
    fclose(file);
...
void HBXXH64::finalize()
{
    ...
    mDigest = XXH3_64bits_digest(static_cast<XXH3_state_t*>(mState));
    XXH3_freeState(static_cast<XXH3_state_t*>(mState));
    mState = NULL;
}
...
{
    return mState ? XXH3_64bits_digest(static_cast<XXH3_state_t*>(mState)) : mDigest;
}
...
LLUUID HBXXH128::digest(const void* buffer, size_t len)
{
    ...
    LLUUID id;
    U64* data = reinterpret_cast<U64*>(id.mData);
...
{
    XXH128_hash_t hash = XXH3_128bits(static_cast<const void*>(str), strlen(str));
    LLUUID id;
    U64* data = reinterpret_cast<U64*>(id.mData);
    // Note: we do not check endianness here and we just store in the same
    // order as XXH128_hash_t, that is low word "first".
...
{
    XXH128_hash_t hash = XXH3_128bits(static_cast<const void*>(str.c_str()), str.size());
    LLUUID id;
    U64* data = reinterpret_cast<U64*>(id.mData);
    // Note: we do not check endianness here and we just store in the same
    // order as XXH128_hash_t, that is low word "first".
...
{
    XXH128_hash_t hash = XXH3_128bits(buffer, len);
    U64* data = reinterpret_cast<U64*>(result.mData);
    // Note: we do not check endianness here and we just store in the same
    // order as XXH128_hash_t, that is low word "first".
    data[0] = hash.low64;
    data[1] = hash.high64;
...
{
    mState = static_cast<void*>(XXH3_createState());
    if (!mState || XXH3_128bits_reset(static_cast<XXH3_state_t*>(mState)) != XXH_OK)
    {
        LL_WARNS() << "Failed to initialize state !" << LL_ENDL;
    }
}
...
void HBXXH128::update(const void* buffer, size_t len)
{
    if (mState)
    {
        XXH3_128bits_update(static_cast<XXH3_state_t*>(mState), buffer, len);
    }
    else
    {
        LL_WARNS() << "Cannot update a finalized digest !" << LL_ENDL;
...
void HBXXH128::update(const std::string& str)
{
    if (mState)
    {
        XXH3_128bits_update(static_cast<XXH3_state_t*>(mState), static_cast<const void*>(str.c_str()),
                           str.length());
    }
    else
    {
...
void HBXXH128::update(std::istream& stream)
{
    ...
    while (stream.good())
    {
        stream.read(buffer, BLOCK_LEN);
        len = stream.gcount();
        XXH3_128bits_update(static_cast<XXH3_state_t*>(mState), static_cast<const void*>(buffer), len);
    }
}
...
void HBXXH128::finalize()
{
    ...
    XXH128_hash_t hash = XXH3_128bits_digest(static_cast<XXH3_state_t*>(mState));
    U64* data = reinterpret_cast<U64*>(mDigest.mData);
    // Note: we do not check endianness here and we just store in the same
    // order as XXH128_hash_t, that is low word "first".
    data[0] = hash.low64;
    data[1] = hash.high64;
...
const LLUUID& HBXXH128::digest() const
{
    if (mState)
    {
        ...
        U64* data = reinterpret_cast<U64*>(const_cast<U8*>(mDigest.mData));
        // Note: we do not check endianness here and we just store in the same
        // order as XXH128_hash_t, that is low word "first".
        data[0] = hash.low64;
        data[1] = hash.high64;
