...
struct LLDir_Dummy: public LLDir
{
    ...
    {
        tut_compat::ensure(STRINGIZE(pathname << " was not checked but should have been"),
                           mChecked.find(pathname) != mChecked.end());
    }
    void ensure_not_checked(const std::string& pathname) const
    {
        tut_compat::ensure(STRINGIZE(pathname << " was checked but should not have been"),
                           mChecked.find(pathname) == mChecked.end());
    }
...
TUT_SUITE("LLDir")
{
    TUT_CASE("LLDir::LLDirTest_object_t_test_1")
        // getDirDelimiter
    {
        using namespace tut;
    ...
    TUT_CASE("LLDir::LLDirTest_object_t_test_2")
        // getBaseFileName
    {
        using namespace tut;
    ...
    TUT_CASE("LLDir::LLDirTest_object_t_test_3")
        // getDirName
    {
        using namespace tut;
        std::string delim = gDirUtilp->getDirDelimiter();
    ...
    TUT_CASE("LLDir::LLDirTest_object_t_test_4")
        // getExtension
    {
        using namespace tut;
        std::string delim = gDirUtilp->getDirDelimiter();
   ...
   TUT_CASE("LLDir::LLDirTest_object_t_test_5")
      // LLDirIterator::next
   {
      using namespace tut;
      std::string delim = gDirUtilp->getDirDelimiter();
    ...
    TUT_CASE("LLDir::LLDirTest_object_t_test_6")
    {
        using namespace tut;
    ...
    TUT_CASE("LLDir::LLDirTest_object_t_test_7")
    {
        using namespace tut;
        set_test_name("add()");
        LLDir_Dummy lldir;
