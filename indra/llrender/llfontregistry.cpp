...
void LLFontDescriptor::addFontFile(const std::string& file_name, EFontHinting hinting, const std::string& char_functor)
{
    char_functor_map_t::const_iterator it = mCharFunctors.find(char_functor);
    mFontFiles.push_back(LLFontFileInfo(file_name, hinting, (mCharFunctors.end() != it) ? it->second : nullptr));
}

void LLFontDescriptor::addFontCollectionFile(const std::string& file_name, EFontHinting hinting, const std::string& char_functor)
{
    char_functor_map_t::const_iterator it = mCharFunctors.find(char_functor);
    mFontCollectionFiles.push_back(LLFontFileInfo(file_name, hinting, (mCharFunctors.end() != it) ? it->second : nullptr));
}
    ...
    for (child = node->getFirstChild(); child.notNull(); child = child->getNextSibling())
    {
        ...
        if (child->hasName("file"))
        {
            ...
            if (child->hasAttribute("load_collection"))
            {
                ...
                if (col)
                {
                    desc.addFontCollectionFile(font_file_name, hinting, char_functor);
                }
            }

            desc.addFontFile(font_file_name, hinting, char_functor);
        }
...
LLFontGL *LLFontRegistry::createFont(const LLFontDescriptor& desc)
{
    ...
    std::transform(getUltimateFallbackList().begin(), getUltimateFallbackList().end(), std::back_inserter(font_files),
                   [](const std::string& file_name) { return LLFontFileInfo(file_name, EFontHinting::FORCE_AUTOHINT); });
            ...
            ++font_search_path_it)
        {
            ...
            for (S32 i = 0; i < num_faces; i++)
            {
                ...
                if (fontp->loadFace(font_path, point_size_scale,
                                 LLFontGL::sVertDPI, LLFontGL::sHorizDPI, is_fallback, i, font_file_it->mHinting))
                {
                    is_font_loaded = true;
                    if (is_first_found)
                    {
