...
LLWearable::EImportResult LLViewerWearable::importStream( std::istream& input_stream, LLAvatarAppearance* avatarp )
{
	...
	for (; iter != end; ++iter)
	{
		...
		LLViewerFetchedTexture::ptr_t image = LLViewerTextureManager::instance().getFetchedTexture( textureid );
		if(gSavedSettings.getBOOL("DebugAvatarLocalTexLoadedTime"))
		{
            image->addCallback([te](bool success, LLUUID texture_id, LLViewerFetchedTexture::ptr_t &src_vi, bool final_done) {
                LLVOAvatarSelf::debugOnTimingLocalTexLoaded(success, texture_id, src_vi, final_done, (LLAvatarAppearanceDefines::ETextureIndex)te);
            });
		}
	}
...
void LLViewerWearable::setTexturesToDefaults()
{
	for( S32 te = 0; te < TEX_NUM_INDICES; te++ )
	{
		if (LLAvatarAppearanceDictionary::getTEWearableType((ETextureIndex) te) == mType)
		{
			...
			LLViewerFetchedTexture::ptr_t image = LLViewerTextureManager::instance().getFetchedTexture( id );
...
void LLViewerWearable::writeToAvatar(LLAvatarAppearance *avatarp)
{
	...
	for( S32 te = 0; te < TEX_NUM_INDICES; te++ )
	{
		if (LLAvatarAppearanceDictionary::getTEWearableType((ETextureIndex) te) == mType)
		{
			...
			LLViewerTexture::ptr_t image = LLViewerTextureManager::instance().getFetchedTexture( image_id, params );
...
void LLViewerWearable::copyDataFrom(const LLViewerWearable* src)
{
	...
	for (S32 te = 0; te < TEX_NUM_INDICES; te++)
	{
		if (LLAvatarAppearanceDictionary::getTEWearableType((ETextureIndex) te) == mType)
		{
			...
			{
                image = LLViewerTextureManager::staticCastToFetchedTexture(src->getLocalTextureObject(te)->getImage());
