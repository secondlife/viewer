...
LLToastNotifyPanel::~LLToastNotifyPanel()
{
    ...
    std::ranges::for_each(mBtnCallbackData, [](auto* p) { delete p; });
...
void LLToastNotifyPanel::init( LLRect rect, bool show_images )
{
    ...
    mTextBox->setIsObjectBlockedCallback([](const LLUUID& id, const std::string& name) { return LLMuteList::getInstance()->isMuted(id, name, 0); });
