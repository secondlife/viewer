...
void LLFloaterAuction::onClickSnapshot(void* data)
{
    ...
    if (success)
    {
        ...
        tga_file.write(std::span{tga->getData(), static_cast<size_t>(tga->getDataSize())});
        ...
        j2c_file.write(std::span{j2c->getData(), static_cast<size_t>(j2c->getDataSize())});

        self->mImage = LLViewerTextureManager::getLocalTexture((LLImageRaw*)raw, false);
        gGL.getTexUnit(0)->bind(self->mImage);
        self->mImage->setAddressMode(LLTexUnit::eTextureAddressMode::TAM_CLAMP);
    }
    else
    {
