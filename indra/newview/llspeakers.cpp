...
void LLSpeakerMgr::update(bool resort_ok)
{
        ...
        sorted_speaker_it != mSpeakersSorted.end(); ++sorted_speaker_it)
    {
        ...
        {
            speakerp->mDotColor = lerp(speaking_color, ACTIVE_COLOR, clamp_rescale(static_cast<F32>(recent_speaker_count), -2.f, 3.f, 0.f, 1.f));
...
void LLLocalSpeakerMgr::updateSpeakerList()
{
    ...
    for (speaker_map_t::iterator speaker_it = mSpeakers.begin(); speaker_it != mSpeakers.end(); ++speaker_it)
    {
        ...
        if (speakerp.notNull() && speakerp->mStatus == LLSpeaker::STATUS_TEXT_ONLY)
        {
            ...
            if (!avatarp || dist_vec_squared(avatarp->getPositionAgent(), LLVector3(gAgent.getPositionAgent())) > CHAT_NORMAL_RADIUS * CHAT_NORMAL_RADIUS)
            {
                setSpeakerNotInChannel(speakerp);
            }
        }
