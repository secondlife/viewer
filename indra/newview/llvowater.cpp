...
//  LLVector3 vtx[4];
    ...
    size_x *= static_cast<S32>(llmin(llround(scale.mV[0] / 256.f), 8));
    size_y *= static_cast<S32>(llmin(llround(scale.mV[1] / 256.f), 8));
    ...
    for (y = 0; y < size_y; y++)
    {
        for (x = 0; x < size_x; x++)
        {
            ...
            position_agent.mV[VX] = static_cast<F32>(llround(position_agent.mV[VX]));
            position_agent.mV[VY] = static_cast<F32>(llround(position_agent.mV[VY]));
