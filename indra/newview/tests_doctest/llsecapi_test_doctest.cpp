...
TUT_SUITE("LLSecAPI")
{
    TUT_CASE("LLSecAPI::secapiTestObject_test_1")
    {
        using namespace tut;
        ...
        ensure("Retrieved sectest1 handler should be the same", retrieved_test1_handler == test1_handler);
        ...
        ensure("Retrieved sectest1 handler should be the same", retrieved_test2_handler == test2_handler);
    }
}
