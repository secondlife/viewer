...
#define DEBUG_PBR_METAL           1 // Output: grayscale metal
...
#define DEBUG_PBR_DOT_NV          0 // Output: Normal,ViewDir
#define DEBUG_PBR_DOT_TV          0 // Output: Tangent,ViewDir
#define DEBUG_PBR_DOT_BV          0 // Output: Bitangent,ViewDir
...
uniform sampler2DRect specularRect; // NOTE: For PBR specularRect is a packed Occlusion Roughness Metal
...
//      vec3  reflect0   = vec3(0.04);  // -> incidence reflectance 0.04
        float IOR        = 0.0; // TODO: Set from glTF/glb?
        vec3  reflect0   = calcBaseReflect0(IOR);

        float metal      = packedORM.b;
        vec3  reflect90  = vec3(0);
        vec3  v          = view_dir;
        vec3  n          = norm.xyz;    // z dir
#if DEBUG_PBR_TANGENT1
        vec3  t          = vec3(1,0,0); // x dir
#else
//      vec3  t          = texture2DRect(tangentMap, tc).rg; // TODO: Get Tangent from GBuffer?
#endif
        vec3  b          = cross(n,t);
...
//        colorSpec += calcRadianceGGX       ( dotNV, vScaleBias, perceptualRough, reflect0, specWeight, reflectVN    );
//        colorDiff += calcRadianceLambertian( dotNV, vScaleBias, perceptualRough, reflect0, specWeight, colorDiffuse );
