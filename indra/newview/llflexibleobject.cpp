...
void LLVolumeImplFlexible::updateRenderRes()
{
    ...
    F32 app_angle = ll_round(static_cast<F32>(atan2( mVO->getScale().mV[2]*2.f, drawablep->mDistanceWRTCamera)) * RAD_TO_DEG, 0.01f);

    // Rendering sections increases with visible angle on the screen
    mRenderRes = static_cast<S32>(FLEXIBLE_OBJECT_MAX_SECTIONS*4*app_angle*DEG_TO_RAD/LLViewerCamera::getInstance()->getView());
#endif

    mRenderRes = llclamp(mRenderRes, new_res-1, static_cast<S32>(FLEXIBLE_OBJECT_MAX_SECTIONS));
    ...
    if (drawablep)
    {
        ...
        if (gPipeline.hasRenderDebugFeatureMask(LLPipeline::RENDER_DEBUG_FEATURE_FLEXIBLE))
        {
            ...
            else
            {
                ...
                U32 update_period = static_cast<U32>((llmax(static_cast<S32>((LLViewerCamera::getInstance()->getScreenPixelArea()*0.01f/(pixel_area*(sUpdateFactor+1.f)))),0)+1));
