...
namespace tut
{
    ...
    struct sechandler_basic_test
    {
        ...
        sechandler_basic_test()
        {
            ...
            BIO * validation_bio;
            validation_bio = BIO_new_mem_buf(reinterpret_cast<const void*>(mPemTestCert.c_str()), static_cast<S32>(mPemTestCert.length()));
            PEM_read_bio_X509(validation_bio, &mX509TestCert, 0, NULL);
            BIO_free(validation_bio);

            validation_bio = BIO_new_mem_buf(reinterpret_cast<const void*>(mPemRootCert.c_str()), static_cast<S32>(mPemRootCert.length()));
            PEM_read_bio_X509(validation_bio, &mX509RootCert, 0, NULL);
            BIO_free(validation_bio);

            validation_bio = BIO_new_mem_buf(reinterpret_cast<const void*>(mPemIntermediateCert.c_str()), static_cast<S32>(mPemIntermediateCert.length()));
            PEM_read_bio_X509(validation_bio, &mX509IntermediateCert, 0, NULL);
            BIO_free(validation_bio);

            validation_bio = BIO_new_mem_buf(reinterpret_cast<const void*>(mPemChildCert.c_str()), static_cast<S32>(mPemChildCert.length()));
