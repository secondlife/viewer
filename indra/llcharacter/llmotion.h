...
public:
    ...
    LLMotionInitStatus onInitialize(LLCharacter*) override { LL_INFOS() << "LLTestMotion::onInitialize()" << LL_ENDL; return LLMotionInitStatus::STATUS_SUCCESS; }
    bool onActivate() override { LL_INFOS() << "LLTestMotion::onActivate()" << LL_ENDL; return true; }
    bool onUpdate(F32 time, U8* joint_mask) override { LL_INFOS() << "LLTestMotion::onUpdate(" << time << ")" << LL_ENDL; return true; }
    void onDeactivate() override { LL_INFOS() << "LLTestMotion::onDeactivate()" << LL_ENDL; }
};
...
public:
    ...
    // motions must report their total duration
    F32 getDuration() override { return 1.f; }
    ...
    // called after parameters have been set
    // must return true to indicate success and be available for activation
    LLMotionInitStatus onInitialize(LLCharacter *character) override { return LLMotionInitStatus::STATUS_SUCCESS; }
    ...
    // must return true while it is active, and
    // must return false when the motion is completed.
    bool onUpdate(F32 activeTime, U8* joint_mask) override { return true; }
